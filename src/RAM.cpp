...
exdata_pair RAM::get_type_and_adress(CharArrayConstView arg) const
{
    ...
    ::byte adress = 0;
    {
        ...
        adress = static_cast<byte>(get_val(buf));
    }

    while (arg.size() != 0) {
        adress = static_cast<byte>(read_val({ get_exdtype(arg[arg.size() - 1]), adress }));
    }
    return { type, adress };
}

dbyte RAM::solve_val(CharArrayConstView arg) const
{
    Stack<dbyte, ::byte> stack(static_cast<byte>(read_num_ref(arg)));
    while (arg.size() != 0) {
...
dbyte RAM::read_val(const exdata_pair& t_and_a) const
{
...
void RAM::set_val(const exdata_pair& t_and_a, dbyte val)
{
    switch (t_and_a.first)
    {
    case exdata_type::B:
    ...
    case exdata_type::L:
        ...
        break;
    case exdata_type::CR:
        t_leds.set(t_and_a.second, static_cast<byte>(val), LedsController32::red);
        break;
    case exdata_type::CB:
        t_leds.set(t_and_a.second, static_cast<byte>(val), LedsController32::blue);
        break;
    case exdata_type::CG:
        t_leds.set(t_and_a.second, static_cast<byte>(val), LedsController32::green);
        break;
    }
}

dbyte RAM::get_val(CharArrayConstView arg) const
{
